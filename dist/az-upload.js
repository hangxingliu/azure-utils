...
var load1 = __swcpack_require__.bind(void 0, function(module, exports) {
    ...
    exports.getContentTypeByExt = getContentTypeByExt;
    const map = {
...
var load5 = __swcpack_require__.bind(void 0, function(module, exports) {
    ...
    function azPutBlob(args) {
        ...
        else if (args.contentType === true) contentType = (0, _contentType1).getContentTypeByExt(path1.extname(args.file));
        ...
        return new Promise((resolve, reject)=>{
            ...
            logger1.log(`request put blob api uri="${apiPath}" size=${size} content-type="${contentType}" ...`);
            const req = (0, _https).request({
                ...
                res.on('end', ()=>{
                    ...
                    logger1.verbose(`api response code=${statusCode} content-type=${resContentType || ''}`);
                    resolve(res.headers);
                });
            });
...
var load7 = __swcpack_require__.bind(void 0, function(module, exports) {
    ...
    function azPutBlockList(args) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            logger3.log(`request put block list api uri="${apiPath}" x-ms-blob-content-type="${contentType}" ...`);
            const req = (0, _https).request({
                host: (0, _types).getAzureBlobHost(connect),
                path: apiPath,
                method,
                ...
                res.on('end', ()=>{
                    if (statusCode !== 201) return rejectWithLog(`HTTP status code is ${statusCode} but not 201`, data);
                    logger3.verbose(`api response code=${statusCode} content-type=${resContentType || ''}`);
                    resolve(res.headers);
                });
            });
            req.on('error', rejectWithLog);
...
var load14 = __swcpack_require__.bind(void 0, function(module, exports) {
    ...
    function fileStat(file) {
        ...
        try {
            ...
            throw new Error(`Get stat of "${path2.basename(file)}" failed: ${error.message}`);
        }
        if (!stat.isFile()) throw new Error(`"${path2.basename(file)}" is not a file!`);
        return stat;
    }
...
var _putBlob = load5();
var _copyBlob = load6();
var _putBlockList = load7();
var _putBlock = load8();
var _blockHelper = load9();
var _env = load10();
var _crypto = load2();
var _env1 = load11();
var _logger = load12();
var _networkRetry = load13();
var _file = load14();
var _contentType = load1();
